...
function RemotePlayerSprite({ player }) {
    ...
    useFrame((_, dt) => {
        ...
        const depthFactor = MathUtils.clamp((smoothPos.current.z - DEPTH_FAR_Z) / (DEPTH_NEAR_Z - DEPTH_FAR_Z), 0, 1);
    ...
    return (_jsxs(RigidBody, { ref: bodyRef, type: "kinematicPosition", colliders: false, position: spawnPosition.current, enabledRotations: [false, false, false], children: [_jsx(CuboidCollider, { args: [0.55, 0.95, 0.18], friction: 0.05, restitution: 0 }), _jsx(DavidSprite, { animation: animation, meshRef: meshRef, isPaused: player.action === "idle" })] }));
}
