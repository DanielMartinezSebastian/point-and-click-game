...
function interpolate(
  ...
  return template.replace(INTERP_RE, (_match, name: string) => {
    const v = vars[name];
    return v !== undefined ? String(v) : `{{${name}}}`;
  });
}

function lookupFirstPhrase(
  key: DialogKey,
  locale: Locale,
): string | undefined {
  const dict = getDictionary(locale);
  const entry = dict?.[key];
  if (!entry || entry.phrases.length === 0) return undefined;
...
export function getRandomPhrase(
  ...
  if (!finalEntry || finalEntry.phrases.length === 0) return key;
